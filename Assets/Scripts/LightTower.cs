...
public class LightTower : MonoBehaviour {
	...
	void Fire()
	{
		Transform Ligths = this.transform.FindChild("ParticleSpawn");
		GameObject instantiatedProjectile  = Instantiate(projPrefab, Ligths.transform.position, Ligths.rotation) as GameObject;
		instantiatedProjectile.GetComponent<Rigidbody>().AddForce(transform.forward * 1000.0f);
		AManager.SendMessage("LaserSound",SendMessageOptions.DontRequireReceiver);
